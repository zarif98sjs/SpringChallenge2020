...
//                            }
                            ...
                            if(distance>1)
                                vpell[myPac.id].push_back(Pellet(i,j,ASE[i][j],distance));
                            continue;
                        }
                        if(distance!=0)
                            vpell[myPac.id].push_back(Pellet(i,j,ASE[i][j],distance));
...
//        }
        ...
        for(auto myPac:myPacsNow)
        {
            ...
            for(auto en:enPacsNow)
            {
                if(isPacInDanger(myPac.r,myPac.c,en.r,en.c,myPac.type,en.type)!="-1")
                    isDanger = true;
            }

            int go_id = 0;

            if(isDanger) go_id = vpell[myPac.id].size()/2;
            if(isDanger) cerr<<"DANGERRRRRR"<<endl;
