...
string isPacInDanger(int mr,int mc,int er,int ec,string mtype,string etype)
{
    ...
    if(mr-er==0 && abs(mc-ec)>9)
        return "-1";

    if(mc-ec==0 && abs(mr-er)>9)
        return "-1";
...
public:
    ...
    Pellet(int r,int c,int val,int dist,int pr)
    {
        this->r = r;
        this->c = c;
        this->val = val;
...
{
    if(A.val==B.val)
        return A.dist*B.pr < B.dist*A.pr;
...
int main()
{
    ...
    for(int i=0;i<height;i++)
        for(int j=0;j<width;j++)
            if(grid[i][j]!='#') ASE[i][j] = 0 , PR[i][j] = 1e8;
    ...
    while (1)
    {
        ...
        for(auto en:enPacsNow) /** inefficient **/
        {
            for(int i=-1;i<=1;i++)
            {
                for(int j=-1;j<=1;j++)
                {
                    ...
                    if(isSafe(nr,nc,0,0,height,width))
                    {
                        PR[nr][nc] -= 100;
                    }
                }
        ...
        for (int i = 0; i < visiblePelletCount; i++)
        {
            ...
            if(value==10)
            {
                ...
                if(min_d!=0)
                    vpell[pid].push_back(Pellet(r,c,value,min_d,PR[r][c]));
...
//                            }
                            ...
                            if(distance>1)
                                vpell[myPac.id].push_back(Pellet(i,j,1,distance,PR[i][j]));
                            continue;
                        }
                        if(distance!=0)
                            vpell[myPac.id].push_back(Pellet(i,j,1,distance,PR[i][j]));
        ...
        for(auto en:enPacsNow)
        {
            for(auto me:myPacsNow)
            {
                ...
                vpell[me.id].push_back(Pellet(en.r,en.c,0,distance,0));
            }
        }
...
//        }
        ...
        for(auto myPac:myPacsNow)
        {
            ...
            if(len==0)
                cout<<"MOVE "<<myPac.id<<" "<<vpell[myPac.id][0].c<<" "<<vpell[myPac.id][0].r<<" "<<myPac.c<<","<<myPac.r;
            else
                cout<<"|MOVE "<<myPac.id<<" "<<vpell[myPac.id][0].c<<" "<<vpell[myPac.id][0].r<<" "<<myPac.c<<","<<myPac.r;
            len++;
        }
