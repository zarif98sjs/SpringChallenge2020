...
int main()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < visiblePacCount; i++)
        {
            ...
            if(mine==1)
                myPacs.push_back(Pac(pacId,mine,x,y));
        }
        ...
        if(game==1)
        {
            ...
            for(auto myPac:myPacs)
            {
                if(t%2==1) sort(ALL(vpell[myPac.id]),cmpPel);
                else sort(ALL(vpell[myPac.id]),cmpPel2);
                t++;
            }
        ...
        for(auto myPac:myPacs)
        {
            int go_x = vpell[myPac.id][nowid[myPac.id]].x;
            int go_y = vpell[myPac.id][nowid[myPac.id]].y;

            if(len==0) cout<<"MOVE "<<myPac.id<<" "<<go_x<<" "<<go_y;
            else cout<<"|MOVE "<<myPac.id<<" "<<go_x<<" "<<go_y;
