...
string isPacInDanger(int mr,int mc,int er,int ec,string mtype,string etype)
{
    ...
    if(mtype=="ROCK" && etype=="SCISSORS") return "-1";
    if(mtype=="SCISSORS" && etype=="PAPER") return "-1";
    if(mtype=="PAPER" && etype=="ROCK") return "-1";

    if(mtype=="ROCK" && etype=="ROCK") return "PAPER";
    if(mtype=="ROCK" && etype=="PAPER") return "SCISSORS";

    if(mtype=="SCISSORS" && etype=="SCISSORS") return "ROCK";
    if(mtype=="SCISSORS" && etype=="ROCK") return "PAPER";

    if(mtype=="PAPER" && etype=="PAPER") return "SCISSORS";
    if(mtype=="PAPER" && etype=="SCISSORS") return "ROCK";
...
int main()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < visiblePacCount; i++)
        {
            ...
            if(mine==1)
            {
                if(abilityCooldown==0)speedHappen[pacId] = true ,SPEEEED = true;
                if(speedTurnsLeft==0)isInSpeed[pacId] = false;
        ...
        for(auto myPac:myPacsNow)
        {
            ...
            {
                if(len==0) cout<<"SWITCH "<<myPac.id<<" "<<convertToType[myPac.id];
                else cout<<"|SWITCH "<<myPac.id<<" "<<convertToType[myPac.id];
                len++;
                changeHoyeGese = true;
            }
        }
