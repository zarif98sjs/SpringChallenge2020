...
bool isPacInDangerOnlyType(string mtype,string etype)
{
...
string isPacInDanger(int m_id,int er,int ec,string mtype,string etype)
{
    cerr<<m_id<<" : "<<er<<","<<ec<<"->"<<dist[m_id][er][ec]<<endl;

    if(dist[m_id][er][ec]>20)
        return "-1";
...
//        }
        ...
        for(auto me:myPacsNow)
        {
            for(auto en:enPacsNow)
            {
                string whatToDo = isPacInDanger(me.id,en.r,en.c,me.type,en.type);
                if(whatToDo!="-1")
                {
...
//                            }
        ...
        for(auto en:enPacsNow)
        {
            for(auto me:myPacsNow)
            {
                ...
                if(isPacInDangerOnlyType(me.type,en.type)==false)
